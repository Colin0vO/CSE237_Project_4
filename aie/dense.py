...
def dense_single_core(input0, input1, output):
    # Use only tile sizes
    ...
    tap_A_L2L1 = TensorTiler2D.group_tiler(
        (m, k),            # full tile dims
        (r, s),            # microkernel tile
        (m // r, k // s),  # number of microtiles
    )[0]
    ...
    tap_B_L2L1 = TensorTiler2D.group_tiler(
        (k, n),
        (s, t),
        (k // s, n // t),
    )[0]
    ...
    def core_fn(of_a, of_b, of_c, dense_kernel):
        # Exactly one output tile
        for _ in range_(1):
            elem_out = of_c.acquire(1)

            # Initialize C to zero (kernel accumulates into this)
            for i in range_(m):
                for j in range_(n):
                    elem_out[i, j] = 0
            ...
            for _ in range_(1):
                ...
                dense_kernel(elem_in_a, elem_in_b, elem_out)
                of_a.release(1)
                of_b.release(1)
    ...
    worker = Worker(
        core_fn,
        [fifo_A_L2L1.cons(), fifo_B_L2L1.cons(), fifo_C_L1L2.prod(), dense_kernel],
    )
...
def main():
    ...
    # Use small range to avoid overflow differences.
    X = iron.randint(-1, 2, (m, k), dtype=element_type, device="npu")  # input
    W = iron.randint(-1, 2, (k, n), dtype=element_type, device="npu")  # weights
    Y = iron.zeros(m * n, dtype=element_type, device="npu")            # output

    # Reference dense layer: Y = ReLU(X @ W)
    ref = np.matmul(X.numpy(), W.numpy())
    ref = np.maximum(ref, 0)           # ReLU
    ref = ref.astype(np.int8)          # match hardware dtype

    dense_single_core(X, W, Y)

    e = np.equal(ref.flatten(), Y.numpy())
